...
// getSmallDetails("Swordfish_Mk_I");
...
async function checkDB() {
  ...
  Airplane.countDocuments({ nation: "Britain" })
    .exec()
    .then((item) => console.log(item));
}

// main body of the parser
// checks if an item is missing and adds it
// useful for when connection error or crash is encountered
async function checkMissingItems(targetURL, input_nation) {
  const url = `https://wiki.warthunder.com/Category:${targetURL}`;
  mongoose.set("strictQuery", false);
  const mongoDB = process.env.MONGODB_URL;
  main()
  ...
  try {
    ...
    for (let y = 0; y < array.length; y++) {
      let planes = await Airplane.find({ name: array[y] });

      if (planes[0] == undefined) {
        newHolder.push(array[y]);
      } else if (planes[0].name == undefined) {
        console.log("undef found");
      }
    }
    console.log("Missing Items:  " + newHolder.length);

    if (newHolder.length > 0) {
      for (let x = 0; x < newHolder.length; x++) {
        let details = await getSmallDetails(urlNameClean(newHolder[x]));
        let airplane = new Airplane({
          name: newHolder[x],
          vehicle_img: await getVehicleImage(urlNameClean(newHolder[x])),
          dynamic_url: await getDynamicImageURL(urlNameClean(newHolder[x])),
          turn_time_RB: details.turn_rate,
          max_speed_RB: details.speed,
          max_climb_RB: details.climb_rate,
          speed_alt: details.speed_alt,
        ...
        console.log(x + 1 + " aircraft out of " + newHolder.length);
        // console.log(airplane);

        await airplane.save();
      }
      console.log("Missing entries added corrected:" + " " + newHolder.length);
      console.log("Saving to DB complete");
      mongoose
        .disconnect(process.env.MONGODB_URL)
        .then(() => console.log("MongoDB disconnected"));
...
// checkMissingItems("USA_aircraft", "USA");
// checkMissingItems("Britain_aircraft", "Britain");
// checkMissingItems("Germany_aircraft", "Germany");
// checkMissingItems("USSR_aircraft", "USSR");
// checkMissingItems("China_aircraft", "China");
// checkMissingItems("Japan_aircraft", "Japan");
// checkMissingItems("Italy_aircraft", "Italy");
// checkMissingItems("France_aircraft", "France");
// checkMissingItems("Sweden_aircraft", "Sweden");
// checkMissingItems("Israel_aircraft", "Israel");
// checkDB();
