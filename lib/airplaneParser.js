...
async function getSmallDetails(vehicleName) {
  if (vehicleName === "MQ-1") {
    ...
    try {
      ...
      const turn_rate_stock = conversionHelpers.nullChecker(
        table.find("tbody > tr:nth-child(3) > td:nth-child(6)").html().trim()
      );
      // upgraded turn rate location
      const turn_rate_upgraded = conversionHelpers.nullChecker(
        table.find("tbody > tr:nth-child(4) > td:nth-child(5)").html().trim()
      );
      // max speed unupgraded
      const max_speed_stock = conversionHelpers.nullChecker(
        table.find("tbody > tr:nth-child(3) > td:nth-child(3)").html().trim()
      );
      // max speed upgraded
      const max_speed_upgraded = conversionHelpers.nullChecker(
        table.find("tbody > tr:nth-child(4) > td:nth-child(3)").html().trim()
      );
      // max specific alt for max speed
      const max_speed_alt = conversionHelpers.nullChecker(
        table.find("tbody > tr:nth-child(1) > th:nth-child(2)").html().trim()
      );
