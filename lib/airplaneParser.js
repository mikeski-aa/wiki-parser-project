  ...
  if (
    ...
    try {
      ...
      const max_climb_upgraded = table
        ...
        .html();
      console.log(max_climb_stock);
      console.log(max_climb_upgraded);
      // RB rating
      const rating = $(
      ...
      // to exclude bundle vehicles from showing up as GJN store ones
      ...
      let categoriesClean = categoriesRaw
        .text()
        .split("PREMIUM")[1]
        .replaceAll("\n", "")
        .split(/(?=[A-Z])/);

      console.log(categoriesClean);
...
getSmallDetails("MiG-21_Bison_(Great_Britain)");
