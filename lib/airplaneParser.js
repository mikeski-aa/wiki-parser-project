...
async function getSmallDetails(vehicleName) {
  ...
  try {
    ...
    const max_speed_alt = table
      ...
      .html()
      .trim();
    console.log("speed upgraded: " + " " + max_speed_upgraded);

    // using conversion helper to manage all speeds that are written in x,xxx format instead of xxxx
    const specs = {
      turn_stock: conversionHelpers.checkForNullNan(turn_rate_stock),
      turn_upgrade: conversionHelpers.checkForNullNan(turn_rate_upgraded),
